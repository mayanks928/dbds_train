...
try {
	...
	+ "EXISTS (SELECT 1 FROM Ticket WHERE reservation_no = r.reservation_no AND activatedAt IS NOT NULL) AS hasActivatedTickets "
	+ "FROM Reservation r " + "JOIN Station s1 ON r.departure_from = s1.station_id "
...
<div class="container my-5">
	...
	while (rsReservations.next()) {
		...
		resv.put("hasActivatedTickets", rsReservations.getBoolean("hasActivatedTickets"));
	...
	if (!hasReservations) {
	%>
	<div class="alert alert-secondary text-center">No reservations
		found.</div>
	<%
	} else {
	%>
		...
		<table class="table table-striped align-middle">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<%
						boolean hasActivatedTickets = (boolean) resv.get("hasActivatedTickets");
						%>
						<%
						if (!isCancelled && !hasActivatedTickets) {
						%>
 ...
 } else if (hasActivatedTickets) {
 %>
						<button class="btn btn-danger btn-sm" disabled>Cancel</button>
						<div class="small text-muted">1 ticket already activated.
							Cannot cancel.</div> <%
 }
 %>
						...
						<form action="viewTickets.jsp" method="get" class="d-inline">
							...
							<button type="submit" class="btn btn-secondary btn-sm">View
								Tickets</button>
						</form>
					</td>
				</tr>
...
e.printStackTrace();
out.println(
		"<div class='container my-5'><div class='alert alert-danger'>Error retrieving reservations. Please try again later.</div></div>");
} finally {
