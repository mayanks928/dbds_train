<%@ page
	import=" com.example.util.DBConnectionUtil,java.sql.Connection, java.sql.Statement, java.sql.ResultSet, java.util.List, java.util.ArrayList"%>
...
<div class="container my-5" style="max-width: 600px;">
	...
	<form action="findTrain.jsp" method="GET"
		...
		<div class="mb-3">
			<label for="origin" class="form-label">Origin</label> <select
				id="origin" name="origin" required class="form-select"
				...
				<option value="<%=stationId%>"
					<%=(request.getParameter("origin") != null && request.getParameter("origin").equals(String.valueOf(stationId)))
		? "selected"
		: ""%>>
					<%=stationName%>
				</option>
...
<form action="searchTrains.jsp" method="GET" class="card p-4 shadow-sm">
	...
	<div class="mb-3">
		<label for="destination" class="form-label">Destination</label> <select
			id="destination" name="destination" required class="form-select">
			...
			if (originId != null) {
				...
				String destinationQuery = "SELECT DISTINCT s.station_id, s.station_name "
						...
						+ "WHERE ts.transit_id IN (" + transitIdList + ") AND ts.station_id != " + originId
	...
	<div class="mb-3">
		<label for="date" class="form-label">Date</label> <input type="date"
			id="date" name="date" required class="form-control">
	</div>
